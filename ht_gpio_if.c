...
GPIO_IF_ButtonConfigure(unsigned char ucPins){
	if(ucPins & BTN1){
		GPIO_IF_GetPortNPin(GPIO_BTN1,
						&g_uiBTN1Port,
						&g_ucBTN1Pin
						);
		GPIO_IF_ConfigureNIntEnable(g_uiBTN1Port,
						g_ucBTN1Pin,
						GPIO_RISING_EDGE,
						Button1Interrupt
						);
...
//*****************************************************************************
unsigned char
GPIO_IF_ButtonRead(unsigned char btnNum){
	unsigned char ucBtnStatus;
	switch(btnNum){
	case BTN1:
		ucBtnStatus = GPIO_IF_Get(GPIO_BTN1, g_uiBTN1Port, g_ucBTN1Pin);
		break;
	case BTN2:
		ucBtnStatus = GPIO_IF_Get(GPIO_BTN2, g_uiBTN2Port, g_ucBTN2Pin);
		break;
	default:
		ucBtnStatus = 0;
	}
...
//*****************************************************************************
void
GPIO_IF_LedToggle(unsigned char ucLedNum)
{
...
Button1Interrupt(){
	...
	ulStatus = MAP_GPIOIntStatus(g_uiBTN1Port,1);
	MAP_GPIOIntClear(g_uiBTN1Port,ulStatus);
